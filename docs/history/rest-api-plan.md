...
└────────┬────────┘    └───────────────┬──────────────────┘
    ...
    │  default  → Streamable HTTP on :8500    │
    │                                         │
...
```
vire-server      always running     :8501   REST API + HTML UI
vire-mcp         always running     :8500   Streamable HTTP MCP (for Claude Code)
...
func main() {
    ...
    if *stdio {
        ...
        // Streamable HTTP transport — listens on :8500
...
services:
  ...
  vire-mcp:
    ...
    entrypoint: ["./vire-mcp"]
    ports:
      - "${VIRE_MCP_PORT:-8500}:8500"
    environment:
      - VIRE_SERVER_URL=http://vire-server:8501
    depends_on:
      vire-server:
...
- `VIRE_SERVER_URL` uses Docker internal DNS (`http://vire-server:8501`)
