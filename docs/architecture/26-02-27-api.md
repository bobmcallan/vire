...
Feedback items support media attachments (base64-encoded). Submit and update accept an optional `attachments` array; each attachment has `filename`, `content_type`, and `data` (base64). Validation: max 10 attachments, 5MB each, allowed types: image/png, image/jpeg, image/gif, image/webp, application/json, text/csv, text/plain. On update, `attachments` is pointer-to-slice: nil preserves existing, empty slice clears.
