...
// TestSyncPortfolio_PreservesExternalBalances_StaleSchema verifies that external
// balances survive a re-sync even when the stored portfolio has a stale schema version.
// The fix: SyncPortfolio uses raw UserDataStore.Get instead of getPortfolioRecord
// (which rejects stale schema versions) to read back external balances.
func TestSyncPortfolio_PreservesExternalBalances_StaleSchema(t *testing.T) {
	today := time.Now()
		...
		holdings: []*models.NavexaHolding{
			{
				...
				Name: "BHP Group", Units: 100,
				CurrentPrice: 45.00, MarketValue: 4500.00, LastUpdated: today,
			},
		},
		trades: map[string][]*models.NavexaTrade{
			"100": {{ID: "1", HoldingID: "100", Symbol: "BHP", Type: "buy", Units: 100, Price: 40.0}},
		},
	}
	...
	// Pre-seed a portfolio with external balances and a STALE schema version
	stalePortfolio := &models.Portfolio{
		Name:               "SMSF",
		DataVersion:        "0", // intentionally stale
		TotalValueHoldings: 4500,
		TotalValue:         54500,
		ExternalBalances: []models.ExternalBalance{
			{ID: "eb_12345678", Type: "cash", Label: "ANZ Cash", Value: 50000},
		},
	...
	// External balances should be preserved despite stale schema
	if len(portfolio.ExternalBalances) != 1 {
		t.Fatalf("expected 1 external balance, got %d", len(portfolio.ExternalBalances))
	}
	if portfolio.ExternalBalances[0].Label != "ANZ Cash" {
		t.Errorf("external balance label = %q, want %q", portfolio.ExternalBalances[0].Label, "ANZ Cash")
	}
	if portfolio.ExternalBalanceTotal != 50000 {
		t.Errorf("ExternalBalanceTotal = %v, want 50000", portfolio.ExternalBalanceTotal)
	}
	// TotalValue should be holdings + external balances
	if portfolio.TotalValue != portfolio.TotalValueHoldings+50000 {
		t.Errorf("TotalValue = %v, want TotalValueHoldings(%v) + 50000",
			portfolio.TotalValue, portfolio.TotalValueHoldings)
	}
}

// TestSyncPortfolio_RecomputesExternalBalanceTotal verifies that SyncPortfolio
// recomputes ExternalBalanceTotal from the ExternalBalances array rather than
// trusting the stored total. Guards against corrupted/inconsistent storage data.
func TestSyncPortfolio_RecomputesExternalBalanceTotal(t *testing.T) {
	today := time.Now()
	...
	// Pre-seed a portfolio with INCONSISTENT external balance data:
	// Array has 30000 total but ExternalBalanceTotal says 99999
	stalePortfolio := &models.Portfolio{
		Name:        "SMSF",
		DataVersion: common.SchemaVersion,
		ExternalBalances: []models.ExternalBalance{
			{ID: "eb_aaaaaaaa", Type: "cash", Label: "ANZ Cash", Value: 20000},
			{ID: "eb_bbbbbbbb", Type: "term_deposit", Label: "TD", Value: 10000},
		},
		ExternalBalanceTotal: 99999, // deliberately wrong
	}
	data, _ := json.Marshal(stalePortfolio)
	_ = userDataStore.Put(context.Background(), &models.UserRecord{
		UserID:  "default",
	...
	// Total should be recomputed from array: 20000 + 10000 = 30000, NOT 99999
	if portfolio.ExternalBalanceTotal != 30000 {
		t.Errorf("ExternalBalanceTotal = %v, want 30000 (recomputed from array, not stored total)",
			portfolio.ExternalBalanceTotal)
	}
	if portfolio.TotalValue != portfolio.TotalValueHoldings+30000 {
		t.Errorf("TotalValue = %v, want TotalValueHoldings(%v) + 30000",
			portfolio.TotalValue, portfolio.TotalValueHoldings)
	}
}
