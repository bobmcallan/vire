...
func TestStress_PopulateChanges_ZeroPortfolioValues(t *testing.T) {
	...
	require.NotNil(t, portfolio.Changes)
	// With zero previous, HasPrevious should be false
	assert.False(t, portfolio.Changes.Yesterday.EquityValue.HasPrevious)
}
...
func TestStress_PopulateChanges_NegativePortfolioValue(t *testing.T) {
	...
	tl := &stressMockTimelineStore{
		snapshots: []models.TimelineSnapshot{
			{Date: yesterday, EquityValue: -5000, PortfolioValue: -3000, GrossCashBalance: -2000, CumulativeDividendReturn: 0},
		},
	}
	...
	require.NotNil(t, portfolio.Changes)
	// Negative previous > 0 should still set HasPrevious true
	// But the requirement says "previous > 0" sets HasPrevious
	// This is a design decision - negative values won't have HasPrevious
	assert.False(t, portfolio.Changes.Yesterday.EquityValue.HasPrevious, "Negative previous should not set HasPrevious")
}
