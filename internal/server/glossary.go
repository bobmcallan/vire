	...
	return models.GlossaryCategory{
		...
		Terms: []models.GlossaryTerm{
			...
			},
			{
				Term:       "total_capital",
				Label:      "Total Capital",
				Definition: "Total value of all assets: equity holdings plus total cash.",
				Formula:    "total_value_holdings + total_cash",
				Value:      p.TotalValue,
				Example:    fmt.Sprintf("%s + %s = %s", fmtMoney(p.TotalValueHoldings), fmtMoney(p.TotalCash), fmtMoney(p.TotalValue)),
			},
		},
	}
}
