# /vire-execute - Script Runner with Auto-Fix

Execute a script and automatically investigate, fix, and retry if it fails.

## Usage

/vire-execute <script-path> [options]

## Options
- `--max-retries N` — Maximum fix-and-retry attempts (default: 5)

## Examples
- `/vire-execute ./scripts/deploy.sh local`
- `/vire-execute ./scripts/build.sh --clean`
- `/vire-execute ./tests/run_integration.sh --max-retries 3`
